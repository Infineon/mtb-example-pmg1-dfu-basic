...
static void tcpwm_interrupt_handler(void *handler_arg, cyhal_timer_event_t event);
...
{
    /* Timer object */
    cyhal_timer_t timer_isr_control;
    ...
    */
    ...
    /* Initializes TCPWM resource for timer operation */
    result = cyhal_timer_init(&timer_isr_control, (cyhal_gpio_t)CYHAL_NC_PIN_VALUE, NULL);
    CY_ASSERT(result == CY_RSLT_SUCCESS);
    
    /* Set the frequency of the clock driving the timer */
    result = cyhal_timer_set_frequency(&timer_isr_control, TCPWM_CLOCK_FREQUENCY);
    CY_ASSERT(result == CY_RSLT_SUCCESS);
    
    /* Configure timer interrupt */
    cyhal_timer_register_callback(&timer_isr_control, tcpwm_interrupt_handler, NULL);
    
    /* Enable timer interrupt */
    cyhal_timer_enable_event(&timer_isr_control, CYHAL_TIMER_IRQ_TERMINAL_COUNT, TCPWM_INTR_PRIORITY, true);
    ...
    /* Start the timer */
    result = cyhal_timer_start(&timer_isr_control);
    CY_ASSERT(result == CY_RSLT_SUCCESS);
